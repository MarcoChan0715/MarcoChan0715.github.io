...
(function preventDoubleTapZoom() {
    ...
    document.addEventListener('touchend', (event) => {
        const target = event.target.closest('.remote-btn, .header-action-btn, .macro-btn, .pill, .pro-btn, .secondary-btn, .danger-btn, .connect-btn');
        if (!target) return;
        const now = Date.now();
...
if(editLayoutBtn) {
    editLayoutBtn.addEventListener('click', () => {
        ...
        if (isEditMode) {
            ...
            // Device cards use the custom pointer-based drag system so desktop and iPad behave the same.
...
function findDeviceGridInsertion(container, selector, draggingItem, clientX, clientY) {
    const items = Array.from(container.querySelectorAll(selector)).filter(item => (
        item !== draggingItem && item.offsetParent !== null && getComputedStyle(item).display !== 'none'
    ));
    ...
    items.forEach(item => {
        const rect = item.getBoundingClientRect();
        let row = rows.find(r => Math.abs(r.top - rect.top) <= rowTolerance);
        if (!row) {
    ...
    // Easy access to the first slot / top-left position.
    if (clientY <= firstItem.rect.top + firstItem.rect.height * 0.38 &&
        clientX <= firstItem.rect.left + firstItem.rect.width * 0.62) {
        return { target: firstItem.item, before: true };
    }

    // Easy access to dropping at the end of the grid.
    if (clientY >= lastItem.rect.bottom - lastItem.rect.height * 0.12 &&
        clientX >= lastItem.rect.left - 40) {
        return { target: lastItem.item, before: false };
    }

    // Pick the nearest row band first. This makes lower rows much less fussy.
    let chosenRow = rows[0];
    let bestDistance = Infinity;

    rows.forEach(row => {
        const rowTop = Math.min(...row.items.map(entry => entry.rect.top));
        const rowBottom = Math.max(...row.items.map(entry => entry.rect.bottom));
        let distance = 0;
        if (clientY < rowTop) distance = rowTop - clientY;
        else if (clientY > rowBottom) distance = clientY - rowBottom;
    ...
    const rowItems = chosenRow.items;
    const firstInRow = rowItems[0];
    const lastInRow = rowItems[rowItems.length - 1];

    if (clientX <= firstInRow.rect.left + firstInRow.rect.width * 0.35) {
        return { target: firstInRow.item, before: true };
    }

    if (clientX >= lastInRow.rect.left + lastInRow.rect.width * 0.65) {
        return { target: lastInRow.item, before: false };
    }

    for (const entry of rowItems) {
        const centerX = entry.rect.left + entry.rect.width / 2;
        if (clientX < centerX) {
            return { target: entry.item, before: true };
        }
    }
...
function setupTouchReorder(container, selector, mode) {
    ...
    const isDeviceGrid = selector === '.device-card';
    ...
    const getDragVisual = () => dragProxy || draggingItem;

    const applyDragPosition = () => {
        rafId = null;
        const visual = getDragVisual();
        if (!visual) return;
        visual.style.left = `${pendingLeft}px`;
        visual.style.top = `${pendingTop}px`;
    };
    ...
    const finishDrag = () => {
        ...
        if (draggingItem) {
            draggingItem.classList.remove('drag-source-hidden', 'dragging', 'touch-dragging');
        ...
        if (mainContent) mainContent.classList.remove('drag-scroll-lock');
    };
    ...
    const clearTouchDrag = (pointerId = null) => {
        ...
        if (targetPlaceholder && targetPlaceholder.parentNode && visual) {
            const targetRect = targetPlaceholder.getBoundingClientRect();
            visual.style.transition = 'left 160ms ease, top 160ms ease, transform 160ms ease, box-shadow 160ms ease, opacity 160ms ease';
            visual.style.left = `${targetRect.left}px`;
            visual.style.top = `${targetRect.top}px`;
            visual.style.transform = 'scale(1) rotate(0deg)';
            visual.style.opacity = '1';
            cleanupTimer = setTimeout(finishCleanup, 170);
        } else {
            finishCleanup();
        }
    };
    ...
    container.addEventListener('pointerdown', (e) => {
        ...
        if (!isTouchLike && !(isDeviceGrid && isMouse)) return;
        ...
        if (isDeviceGrid) {
            // Use a cloned drag proxy so large cards do not flicker or get clipped by grid reflow.
            item.parentNode.insertBefore(placeholder, item);
            dragProxy = item.cloneNode(true);
            dragProxy.classList.add('touch-dragging', 'drag-proxy');
            ...
            dragProxy.style.zIndex = '4000';
            dragProxy.style.pointerEvents = 'none';
            dragProxy.style.boxSizing = 'border-box';
            dragProxy.style.transition = 'transform 160ms ease, box-shadow 160ms ease';
            dragProxy.style.transform = 'scale(1.02) rotate(-0.6deg)';
            ...
            item.style.transition = 'transform 160ms ease, box-shadow 160ms ease';
        ...
        if (mainContent) mainContent.classList.add('drag-scroll-lock');
    ...
    document.addEventListener('pointermove', (e) => {
        ...
        if (isDeviceGrid) {
            const containerRect = container.getBoundingClientRect();
            const withinGrid = (
                e.clientX >= containerRect.left - 80 &&
                e.clientX <= containerRect.right + 80 &&
                e.clientY >= containerRect.top - 80 &&
                e.clientY <= containerRect.bottom + 80
            );
            if (withinGrid) {
                insertion = findDeviceGridInsertion(container, selector, draggingItem, e.clientX, e.clientY);
            }
        } else {
            ...
            if (hoverItem) {
                ...
                insertion = { target: hoverItem, before: !shouldInsertAfter(mode, rect, e.clientX, e.clientY) };
            }
        }

        if (insertion && insertion.target) {
            if (insertion.before) insertion.target.before(placeholder);
            else insertion.target.after(placeholder);
        }
...
document.body.addEventListener('click', (e) => {
    ...
    if (e.target.closest('#macro-all-off') && !isEditMode) queueLog('Macro executed: All Devices Off.');
    if (e.target.closest('#macro-all-on') && !isEditMode) queueLog('Macro executed: All Devices On.');
    ...
    if (e.target.closest('#macro-eco') && !isEditMode) queueLog('Macro executed: Eco Heating.');
});
