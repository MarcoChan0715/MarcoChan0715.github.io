...
document.getElementById('routines-grid').addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains('run-routine-btn')) {
        ...
        const routineName = btn.closest('.device-card').querySelector('h3').textContent;
...
macrosContainer.addEventListener('dragover', (e) => {
    ...
    if (hoverBtn) {
        ...
        const pointerIsAfter = e.clientY > (bounding.top + bounding.height / 2) ||
                               (Math.abs(e.clientY - (bounding.top + bounding.height / 2)) < bounding.height / 4 && e.clientX > (bounding.left + bounding.width / 2));
        if (pointerIsAfter) hoverBtn.after(draggingBtn);
...
function setupTouchReorder(container, selector, mode) {
    ...
    const sortCandidates = (items) => items.slice().sort((a, b) => {
        ...
        const sameRow = Math.abs(rectA.top - rectB.top) < Math.max(rectA.height, rectB.height) / 2;
        if (sameRow) return rectA.left - rectB.left;
    ...
    const releaseScrollLock = () => {
        document.body.classList.remove('drag-edit-lock');
    ...
    container.addEventListener('pointerdown', (e) => {
        ...
        document.body.classList.add('drag-edit-lock');
    ...
    document.addEventListener('pointermove', (e) => {
        ...
        const candidates = sortCandidates(Array.from(container.querySelectorAll(selector)).filter(item => (
            item !== draggingItem && item !== placeholder
        )));

        if (!hoverItem && candidates.length) {
            const containerRect = container.getBoundingClientRect();
            const withinContainerBounds = (
                e.clientX >= containerRect.left - 60 &&
                e.clientX <= containerRect.right + 60 &&
            ...
            if (withinContainerBounds) {
                ...
                if (mode === 'horizontal') {
                    if (e.clientX <= firstRect.left + firstRect.width * 0.35) {
                        hoverItem = first;
                        forcedPlacement = 'before';
                    } else if (e.clientX >= lastRect.right - lastRect.width * 0.35) {
                        hoverItem = last;
                        forcedPlacement = 'after';
                    }
                } else {
                    if (e.clientY <= firstRect.top + firstRect.height * 0.35 ||
                        (e.clientY <= firstRect.bottom && e.clientX <= firstRect.left + firstRect.width * 0.35)) {
                        hoverItem = first;
                        forcedPlacement = 'before';
                    } else if (e.clientY >= lastRect.bottom - lastRect.height * 0.35 ||
                               (e.clientY >= lastRect.top && e.clientX >= lastRect.right - lastRect.width * 0.35)) {
                        hoverItem = last;
                        forcedPlacement = 'after';
                    }
                }
        ...
        const insertAfter = forcedPlacement === 'after'
            ? true
            : forcedPlacement === 'before'
                ? false
                : shouldInsertAfter(mode, rect, e.clientX, e.clientY);
